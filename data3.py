...
if __name__ == '__main__':
    ...
    a = pd.read_csv(sonar,usecols=list(range(0,60)))
    a1 = pd.read_csv(sonar,usecols=[60])
    a2 = []
    for i, j in enumerate(a1.values):
        if j == 'R':
            a2.append(0)
        else:
            a2.append(1)
    ...
    for iii, iiii in enumerate(bl):
        ...
        # --------------GRF模型-----------------
        sigma0 = np.array(np.array(list(range(1, 20)))/15).tolist()
    ...
    np.savez('re2.npz', acc_GRF=acc_GRF, acc1_GRF=acc1_GRF,
